    ...
    <div className="max-w-5xl mx-auto px-4 py-10 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-1">
          ...
          <p className="text-xl font-bold text-gray-900 capitalize">{profile?.plan ?? 'free'}</p>
          {profile?.plan === 'free' && (
            <Link href="/pricing" className="text-xs text-indigo-600 hover:underline">Upgrade to Pro →</Link>
          )}
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-1">
          ...
          <p className="text-xl font-bold text-gray-900">
            ...
            {profile?.plan === 'free' && <span className="text-sm font-normal text-gray-400"> / 10</span>}
          </p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 p-5 space-y-1">
          <p className="text-xs text-gray-500 uppercase tracking-wide font-medium">Total Evaluations</p>
      ...
      {/* Evaluations list */}
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">Past Evaluations</h2>
        {evals.length === 0 ? (
          <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-12 text-center space-y-4">
            ...
            <p className="text-sm text-gray-400">Start by evaluating your first student essay.</p>
            <Link
              href="/evaluate"
              ...
              <details key={ev.id} className="group">
                <summary className="flex items-center justify-between px-6 py-4 cursor-pointer hover:bg-gray-50 transition-colors list-none">
                  <div className="flex items-center gap-4 min-w-0">
                    <span className={`text-sm font-bold px-2.5 py-1 rounded-lg ${gradeBackground(ev.results.overall_grade)}`}>
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 space-y-4">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      ...
                      { label: 'Argumentation', val: ev.results.argumentation_score },
                      { label: 'Critical Thinking', val: ev.results.critical_thinking_score },
                    ].map((s) => (
                      <div key={s.label} className="bg-white rounded-xl p-3 text-center border border-gray-100">
                        <p className="text-lg font-bold text-gray-900">{s.val}<span className="text-xs text-gray-400">/10</span></p>
                        <p className="text-xs text-gray-500 mt-0.5">{s.label}</p>
                      </div>
                    ))}
                  </div>
                  <p className="text-sm text-gray-700 leading-relaxed">{ev.results.detailed_feedback}</p>
                </div>
              </details>
            ))}
          </div>
