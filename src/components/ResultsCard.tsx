...
const PDFDownloadButton = dynamic(() => import('./PDFReport').then((m) => m.PDFDownloadButton), {
  ssr: false,
  loading: () => (
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      <div className="bg-indigo-600 px-6 py-4 flex items-center justify-between">
        <div>
          <h2 className="text-white font-semibold text-lg">Evaluation Results</h2>
          {studentName && (
            <p className="text-indigo-200 text-sm mt-0.5">Student: {studentName}</p>
          )}
        </div>
        <span
      ...
      <div className="p-6 space-y-8">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <ScoreRadial score={result.argumentation_score} label="Argumentation" />
            <ScoreRadial score={result.critical_thinking_score} label="Critical Thinking" />
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-gray-700 text-sm leading-relaxed">{result.detailed_feedback}</p>
        </div>
